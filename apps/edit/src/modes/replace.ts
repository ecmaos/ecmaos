    ...
    default:
      if (event.key.length === 1 && !event.ctrlKey && !event.metaKey && !event.altKey) {
        ...
        if (state.cursorX < lineLength) {
          state.lines[state.cursorY] = currentLine.slice(0, state.cursorX) + event.key + currentLine.slice(state.cursorX + 1)
        } else {
          state.lines[state.cursorY] = currentLine + event.key
        }
        state.cursorX++
        state.modified = true
      }
